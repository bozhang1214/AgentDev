...
async def chat(request: ChatRequest):
    ...
    print(f"请求 session_id: {request.session_id}, 历史长度: {len(history)}")
    if len(history) > 0:
        print(f"最近一条历史: {history[-1]}")
    ...
    if assistant_message.tool_calls:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"二次调用失败：{str(e)}")
        ...
        sessions[sess_id].append({"role": "assistant", "content": final_reply})

        usage = {
            "promp_tokens": total_prompt_tokens,
            "completion_tokens": total_completion_tokens,
            "total_tokens": total_prompt_tokens + total_completion_tokens
        }
        return ChatResponse(reply=final_reply, session_id=sess_id, usage=usage, tool_called=tool_called)
    else:
        ...
        return ChatResponse(reply=reply, session_id=sess_id, usage=usage, tool_called=False)
